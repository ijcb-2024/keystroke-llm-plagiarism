...
from src.data_preprocessor.keyboard_specific import KeyboardSpecificPreprocessor
...
def train(args):
    ...
    preprocessor = KeyboardSpecificPreprocessor(seq_len, batch_size)

    buffalo_free = preprocessor.process_data_buffalo(buffalo_free)
    buffalo_fixed = preprocessor.process_data_buffalo(buffalo_fixed)
    free_data = preprocessor.process_data(buffalo_free)
    fixed_data = preprocessor.process_data(buffalo_fixed)
    ...
    ## Train Dataset : Keyboard - 3, Test Dataset : Keyboard - 3
